...
local EnvironmentManager_type_def = get_Environment_method:get_return_type();
local getEnvironmentType_method = EnvironmentManager_type_def:get_method("getEnvironmentType(app.FieldDef.STAGE, System.UInt32)");
local EnvironmentSection_field = EnvironmentManager_type_def:get_field("_EnvironmentSection");

local Section_get_Environment_method = EnvironmentSection_field:get_type():get_method("get_Environment");
...
local get_MasterField_method = Constants.GA_type_def:get_method("get_MasterField"); -- static

local MasterFieldManager_type_def = get_MasterField_method:get_return_type();
local get_CurrentStage_method = MasterFieldManager_type_def:get_method("get_CurrentStage");
...
local STAGE_type_def = sdk.find_type_definition("app.FieldDef.STAGE");
local stageNames = {
    "경계의 모래 평원",
    "주홍빛 숲",
    "기름 솟는 계곡",
...
local ENVIRONMENT_type_def = sdk.find_type_definition("app.EnvironmentType.ENVIRONMENT");
...
local function getEnvName(env)
    if env == environments.INVALID then
        return "기본값";
    else
...
local QUALITY_type_def = sdk.find_type_definition("ace.cUpscaleSetting.QUALITY");
local UpscaleNames = {
    "DLAA",
    "품질 우선",
    "균형",
...
local function getCondition()
    local s = get_CurrentStage_method:call(get_MasterField_method:call(nil));
    local EnvironmentManager = get_Environment_method:call(nil);
    local e = Section_get_Environment_method:call(EnvironmentSection_field:get_data(EnvironmentManager));
    if e == environments.INVALID or e == nil then
        e = getEnvironmentType_method:call(EnvironmentManager, s, 0);
    end
    return s, e;
end

local function getStageDefaultGraphicLevel(stage)
    if settings.items ~= nil then
        for _, item in pairs(settings.items) do
            if item.matcher ~= nil and item.matcher.stage == stage and (item.matcher.env == nil or item.matcher.env == environments.INVALID) then
                return item.level;
            end
        end
    end
...
local function calculateGraphicLevel(stage, env)
    if settings.items == nil then
        return 0;
    end
    for _, item in pairs(settings.items) do
        if item.matcher ~= nil and item.matcher.stage == stage and item.matcher.env == env then
            return item.level;
        end
    end
    return getStageDefaultGraphicLevel(stage);
end
...
local function autoAdjust()
    if settings.enabled == true and settings.auto_adjust == true and env ~= nil and env ~= environments.INVALID and stage ~= nil and stage ~= stages.INVALID and inQuest == true then
        local now = os.time();
            ...
            if duration >= settings.auto_adjust_increase_duration then
                ...
                elseif fps > settings.auto_adjust_fps_target + settings.auto_adjust_fps_increase_threshold then
                    if autoAdjustLastReduceTime == nil or now - autoAdjustLastReduceTime >= settings.auto_adjust_recover_interval then
                        local newGraphicLevel = graphicLevel + 1;
                        local recommandLevel = calculateGraphicLevel(stage, env);
                        if newGraphicLevel > recommandLevel + settings.auto_adjust_max_level then
                            newGraphicLevel = recommandLevel + settings.auto_adjust_max_level;
                        end
                        if newGraphicLevel ~= graphicLevel then
                            graphicLevel = newGraphicLevel;
...
local function changeCondition()
    if settings.enabled == true then
        autoAdjustReset();
        local newStage, newEnv = getCondition();
        if newStage ~= nil and newStage ~= stages.INVALID and (stage == nil or newStage ~= stage or env == nil or newEnv ~= env) then
            muted = false;
            stage = newStage;
            env = newEnv;
...
sdk.hook(MasterFieldManager_type_def:get_method("changeStage(app.FieldDef.STAGE)"), nil, changeCondition);

local function questEnd(retval)
    inQuest = false;
    autoAdjustLastReduceTime = nil;
    if settings.enabled == true and settings.fps_message == true and questFrames > 0 and questBeginTime > 0 then
...
for _, t in pairs({"app.cQuestCancel", "app.cQuestClear", "app.cQuestFailed", "app.cQuestResult", "app.cQuestReward"}) do
    sdk.hook(sdk.find_type_definition(t):get_method("enter"), nil, questEnd);
end
...
re.on_draw_ui(function()
    if imgui.tree_node("Dynamic Resolution") == true then
        ...
        changed, settings.fps_message = imgui.checkbox("FPS Avg Report when Quest End", settings.fps_message);
        if changed == true and requireSave ~= true then
            requireSave = true;
        end
        local Option = get_Option_method:call(get_GUI_method:call(nil));
        ...
        changed, uiCurrentStageIdx = imgui.combo("Stage", uiCurrentStageIdx, stageNames);
        if changed == true then
            uiCurrentEnvIdx = 0;
        end
        _, uiCurrentEnvIdx = imgui.combo("Environment", uiCurrentEnvIdx, environmentNames);
        ...
        if selectedStage ~= nil and selectedEnv ~= nil then
            ...
            local thisChanged, value = imgui.slider_int("Graphic Level", graphicLevel, 0 - max_resolution - max_upscale_mode, max_resolution + max_upscale_mode);
        ...
        changed, settings.auto_adjust = imgui.checkbox("Auto Adjust", settings.auto_adjust);
        if changed == true then
            if requireSave ~= true then
                requireSave = true;
            end
        ...
        if settings.auto_adjust == true then
            if imgui.tree_node("Auto Adjust Settings") == true then
                changed, settings.auto_adjust_fps_target = imgui.slider_int("FPS Target", settings.auto_adjust_fps_target, 30, 144);
                if changed == true then
                    if requireSave ~= true then
                        requireSave = true;
                    end
                ...
                changed, settings.auto_adjust_fps_reduce_threshold = imgui.slider_int("FPS Reduce Threshold", settings.auto_adjust_fps_reduce_threshold, 1, 20);
                if changed == true then
                    if requireSave ~= true then
                        requireSave = true;
                    end
                ...
                changed, settings.auto_adjust_fps_increase_threshold = imgui.slider_int("FPS Increase Threshold", settings.auto_adjust_fps_increase_threshold, 0, 20);
                if changed == true then
                    if requireSave ~= true then
                        requireSave = true;
                    end
                ...
                changed, settings.auto_adjust_reduce_duration = imgui.slider_int("Reduce Interval", settings.auto_adjust_reduce_duration, 1, 15);
                if changed == true then
                    if requireSave ~= true then
                        requireSave = true;
                    end
                ...
                changed, settings.auto_adjust_increase_duration = imgui.slider_int("Increase Interval", settings.auto_adjust_increase_duration, settings.auto_adjust_reduce_duration, 30);
                if changed == true then
                    if requireSave ~= true then
                        requireSave = true;
                    end
                ...
                changed, settings.auto_adjust_max_level = imgui.slider_int("Max Adjusted Level", settings.auto_adjust_max_level, 1, 5);
                if changed == true then
                    if requireSave ~= true then
                        requireSave = true;
                    end
        ...
        if imgui.tree_node("Detailed Settings") == true then
            ...
            local thisChanged, max_resolution_idx = imgui.combo("Max Resolution", max_resolution, resolution_names);
            if thisChanged == true then
            ...
            local newValue = nil;
            changed, newValue = imgui.combo("Min Resolution", settings.min_resolution + 1, resolution_names);
            if changed == true then
            ...
            changed, settings.max_upscale_mode = imgui.slider_int("Max Upscale Mode", settings.max_upscale_mode, upscaleValue, max_upscale_mode);
            if changed == true and requireSave ~= true then
                requireSave = true;
            end
            changed, settings.min_upscale_mode = imgui.slider_int("Min Upscale Mode", settings.min_upscale_mode, 0, upscaleValue);
            if changed == true and requireSave ~= true then
                requireSave = true;
            end
            changed, newValue = imgui.combo("Up Level Prefered Option", indexOf(GraphicOptionValues, settings.up_level_prefered_option), GraphicOptions);
            if changed == true then
            ...
            changed, newValue = imgui.combo("Down Level Prefered Option", indexOf(GraphicOptionValues, settings.down_level_prefered_option), GraphicOptions);
            if changed == true then
