...
local TimelineEventDefine_ID_type_def = find_type_definition("app.TimelineEventDefine.ID");
local skipList = {
...
local reqSkip = nil;
Constants.hook(DemoMediator_type_def:get_method("onPlayStart(ace.DemoMediatorBase.cParamBase)"), function(args)
    local ID = get_ID_method:call(to_managed_object(args[3]));
    log.debug(Constants.tostring(ID));
